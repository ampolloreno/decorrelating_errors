...
\amp{Not at all a final draft, but tried to take your note about coherence of noise and write something.}In general, an error will neither be fully coherent nor Pauli stochastic, and there is significant work defining precisely how coherent arbitrary noise is \cite{CarignanDugas2019, Wallman2015}. In particular, the unitarity of an error map can be defined to be:
\begin{align}
u(\mathcal{E}) = \frac{d}{d-1}\int d\psi ||\mathbf{n}[\mathcal{E}\cdot\vectorize{\ketbra{\psi}{\psi}}] - \mathbf{n}[\mathcal{E}\cdot\vectorize{\identmat_d}/2]||^2
\end{align}
where the integral is taken over all pure states and \textbf{n} is the norm of the Bloch vector. Even if we avoid measuring this unitarity, there are protocols to detect how unitary an error is by seeing how errors accumulate on different input states\cite{1907.05950}. In this work, we will be content if the unitarity of our error map decreases by any of these metrics.
%Even if we avoid measuring the unitarity explicitly, we can assess how unitary an error is from how they compound. In particular, coherent errors have the property that they accumulate as the square of the number of qubits. To this end, there are protocols such as that given in \cite{1907.05950} that will use a collection of maximally sensitive inputs to detect this scaling.
