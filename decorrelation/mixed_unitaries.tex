...
\end{equation}
This channel will have a diamond norm proportional to $\epsilon^2$, whereas any other combination will a linear dependence on $\epsilon$. On the otherhand, any linear combination of $\{U_{\epsilon}, U_{-\epsilon}\}$ minimizes the AGI of the resulting channel with a value of $\epsilon^2$.
...
\end{split}
\end{equation}
with $\eta \geq 0$. By making $\eta$ larger, we can supress the diamond norm to second order, while ensuring that we are selecting those that will give us a smaller prefactor. The absolute value of $\eta$ depends on the particular numerical values in $\bold{D}^\ell$.
