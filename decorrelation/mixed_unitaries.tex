...
This example shows that MQGs are capable of outperforming the individual gates of which they are composed. In particular, given a collection of controls whose miscalibration errors are known, this demonstrates that the techniques in Section \ref{sec:convex_programs} can be used to trade the coherent errors for incoherent errors. In future work, we hope to apply this technique to two-qubit gates, which are significantly more difficult to tune up in general and are more likely to possess lingering coherent errors. Tomography or modeling can then be used to construct the process matrix estimates required to build a mixed gate. 
