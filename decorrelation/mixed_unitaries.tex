...
\error =
	...
	\right)
\end{equation}
The top row of all trace-preserving (TP) maps is fixed to $\{1,0,0,0,\cdots\}$.  The rest of the first column, $\vec{m}$, describes any deviations from unitality, as could arise from amplitude damping. If the error channel is unitary, then the error is coherent, and the submatrix $R$ is perfectly antisymmetric, corresponding to a rotation of the generalized Bloch vector. If  $R$ is diagonal, then the error channel is Pauli stochastic, with each entry  corresponding to the probability that the associated Pauli error occurs in each application of the gate. That is, $\error_{ii}$ gives the probability that the following map is applied:
\begin{equation}
\vec\rho_k\rightarrow\sum_j\tr{(\sigma_j^{\dagger}\sigma_i\sigma_k\sigma_i^{\dagger})}\vec{\rho_k}
\end{equation}
...
The problem of minimizing $\sin\theta\cos\theta$ can equivalently be cast as trying to construct a family of vectors, $\sin\theta\cos\theta(\sigma_j^{\dagger}H_i\sigma_k - \sigma_j^{\dagger}\sigma_kH_i)$ whose convex hull contains the origin. This can be seen in Figure \ref{fig:vectorspace} a) and b).
...
= \alpha\mathcal{F}(\error_1) + (1-\alpha)\mathcal{F}(\error_2)
\end{align}
...
We must assume that the 1-norm is uniformly differentiable. In that case, we see that:
...
We begin by generalizing Lemma 2 from \cite{Campbell2017}. From A3 in \cite{Campbell2017}, we know that $||e^{iM}| - (\id + iM)|| \leq \frac{1}{2}||M||^2$.

Thus we can Taylor expand any mixture of unitaries, as:
\begin{equation}
\deriv\sum_j \omega_je^{iH_j} = \deriv(\id + (\sum_ji\omega_jH_j) + \sum \omega_j\sum_{n=2}^{\infty}\frac{(iH_j)^n}{n!}))
\end{equation}
By assumption, the derivatives of the first order term sum to zero, and so we see by A3 that
\begin{equation}
||\deriv\sum_j\omega_je^{iH_j}|| \leq \sum_j\omega_j\frac{c^2}{2} \leq \frac{c^2}{2}
\end{equation}
...
&\leq c + c^2 + \frac{1}{2}c^3
\end{align}
And so we see that Lemma 2 generalizes.
%Where we have used H\"older's inequality. 

Next we generalize the Mixing Lemma from \cite{Campbell2017}:\ldots
