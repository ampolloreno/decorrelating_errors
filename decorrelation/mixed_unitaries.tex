...
In general an error will neither be fully coherent nor Pauli stochastic and so it is useful to define what we mean by the coherence of arbitrary noise\cite{CarignanDugas2019}.
 One way to measure this is with a quanitiy called the unitarity\cite{Wallman2015}:
\begin{align}
...
As quantum devices grow in size, it is increasingly expensive to simulate their dynamics. Coherent errors impose a particular burden on classical simulators, as the entire quantum state must be tracked continually. While there has been significant work done investigating how to approximate different error channels as Pauli stochastic channels, and assessing the quality of these approximations\cite{Puzzuoli2014}, this technique offers another path forward. Pauli-exact MQGs can be constructed with vanishing coherent error, enabling the use of much more efficient Pauli stochastic simulators that are able to model significantly more qubits than their vector-state counterparts. 

\amp{clean up} In particular, the well-known stabilizer framework \cite{quant-ph/9807006} for quantum simluation gives a polynomial-time algorithm to compute expectation values of circuits consisting of only Clifford operations. These circuits are useful in that they give examples of quantum circuits whose performance we can test efficiently against classical simulation. This has motivated significant development of these techniques to simulating stabilizer circuits with mixed state inputs\cite{Aaronson2004} and simulating the evolution of mixtures of stabilizer states. \cite{Yoder, Bennink2017} An interesting property of such simulation techniques is that the cost of the classical simulation is proportional to how non-Clifford the circuit is. Therefore Pauli-Exact MQGs have a clear benefit - if the error channels of a circuit can be made to look more like Pauli-Stochastic errors, they will be more amenable to stabilizer simualtion. Like generator-exact MQGs, the optimal weights can be found efficiently by a convex optimization. 
...
\label{sub:one_qubit}
We consider the following dimensionless model for a single tunable qubit: 
