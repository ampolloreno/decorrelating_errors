...
In general an error will neither be fully coherent nor Pauli stochastic and so it is useful to define what we mean by the coherence of arbitrary noise\cite{CarignanDugas2019}.
...
% ==============================================================================
% contrast coherent errors with stochastic where an expectation can be computed and then the circuit can be rerun, rather than having to track the the whole state.

\subsubsection{Pauli-exact MQGs} % (fold)
\label{sub:off_diagonals}
As quantum devices grow in size, it is increasingly expensive to simulate their dynamics. Coherent errors impose a particular burden on classical simulators, as the entire quantum state must be tracked continually. Existing work generally approximates channels as Pauli stochastic channels, and assesses the quality of these approximations\cite{Puzzuoli2014, Magesan2013}. Pauli frame randomization adds additional gates into circuits leave the result unchanged but randomize the nosie in such a way that it becomes a Pauli stochastic channel. This technique offers another path forward. Pauli-exact MQGs can be constructed with vanishing coherent error and without adding any additional gates or compilation time. Building MQGs with diminished coherent error enables the use of much more efficient Pauli stochastic simulators that are able to model significantly more qubits than their vector-state counterparts. 

In particular, the well-known stabilizer framework \cite{quant-ph/9807006} for quantum simluation gives a polynomial-time algorithm to compute expectation values of circuits consisting of only Clifford operations. These circuits are useful because they give examples of quantum circuits whose performance we can efficiently assess using classical simulation. This has motivated significant development of these techniques, including extensions to simulating stabilizer circuits with mixed state inputs\cite{Aaronson2004} and even algorithms for the simulation of arbitrary circuits. \cite{Yoder, Bennink2017} An interesting property of such simulation techniques is that the cost of the classical simulation is proportional to how non-Clifford the circuit is. Therefore Pauli-Exact MQGs have a clear benefit - if the error channels of a circuit can be made to look more like Pauli Stochastic errors, they will be more amenable to stabilizer simulation. Like generator-exact MQGs, the optimal weights can be found efficiently by a convex optimization. 
