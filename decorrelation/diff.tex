...
%DIF ADD mixed_unitaries.tex         Wed May  6 14:09:31 2020
...
\end{split}
\end{equation}
Here, \DIFdelbegin \DIFdel{$\lambda > 0$ }\DIFdelend \DIFaddbegin \DIFadd{$\lambda \geq 0$ }\DIFaddend is a tunable parameter that can be used to control the degree of sparsity in the solution. \DIFdelbegin \DIFdel{Increasing }\DIFdelend \DIFaddbegin \DIFadd{If }\DIFaddend $\lambda$ \DIFaddbegin \DIFadd{is set to zero, the inner program reduces to Eq.~}\eqref{eq:minimization} \DIFadd{and the outer minimization is redundant. If $\lambda$ is very large each inner minimization problem is minimized for an MQG with a single control, and the outer minimization simply selects the control which has the error generator of smallest norm. While this gives a maximally sparse solution, it doesn't allow for any cancellation of coherent errors. In particular, increasing $\lambda$ }\DIFaddend will increase the sparsity of the solution, but will generally decrease the likelihood that the solution will be generator-exact. 
...
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\linewidth]{sparisity-versus-parameter_kcy.png}
\DIFaddendFL \caption{Sparsity of MQGs as a function of the sparisty parameter $\lambda$\DIFdelbeginFL \DIFdelFL{, with higher weight controls omitted to emphasize the low-weight distribution}\DIFdelendFL . Numerical values of $\lambda_i$ are: $\lambda_0=0$, \DIFdelbeginFL \DIFdelFL{$\lambda_1=6.25$e-05}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\lambda_1=0.0001$}\DIFaddendFL , \DIFdelbeginFL \DIFdelFL{$\lambda_2=0.0001$, $\lambda_3=0.00025$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\lambda_2=0.00025$ }\DIFaddendFL and \DIFdelbeginFL \DIFdelFL{$\lambda_4=0.0005$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\lambda_3=0.0005$}\DIFaddendFL . As the parameter is increased from zero, the number of controls with low probability increases \DIFaddbeginFL \DIFaddFL{(seen on the plot as the probability mass moving to the left)}\DIFaddendFL . For \DIFdelbeginFL \DIFdelFL{$\lambda_4$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\lambda_3$ }\DIFaddendFL virtually all of the 750 controls have vanishingly small probability, allowing for the restriction of the MQG to a small number of controls.}
\label{fig:sparsity}
\end{figure}
...
\DIFdel{) }\DIFdelend \DIFaddbegin \DIFadd{. As $\lambda$ }\DIFaddend increases, the \DIFdelbegin \DIFdel{support of the MQGs become more peaked, with most of their probability mass concentrating on a smaller number of controls . }\DIFdelend \DIFaddbegin \DIFadd{optimal solution contains fewer controls with large support. 
}
