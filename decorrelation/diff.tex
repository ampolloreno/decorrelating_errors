...
%DIF ADD mixed_unitaries.tex         Thu May  7 17:15:13 2020
...
\DIFadd{In general, increasing $\lambda$ decreases the number of controls with large weights, and by removing controls with weights below a threshold probability we can }\DIFaddend increase the sparsity of the solution\DIFdelbegin \DIFdel{, but will generally }\DIFdelend \DIFaddbegin \DIFadd{. Truncating in this way will introduce error in the Pauli-exactness of the solution. For larger $\lambda$ the required threshold probability decreases, and so the error introduced from truncation can be made smaller. However, larger $\lambda$ will also }\DIFaddend decrease the likelihood \DIFdelbegin \DIFdel{that the solution will be }\DIFdelend \DIFaddbegin \DIFadd{of the optimizer returning a }\DIFaddend generator-exact \DIFaddbegin \DIFadd{solution, and so in this way, tuning $\lambda$ allows us to trade between different sources of error when constructing sparse MQGs}\DIFaddend .
