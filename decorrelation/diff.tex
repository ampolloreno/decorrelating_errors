...
%DIF DEL ../../mixed_unitaries.tex   Wed Apr 22 13:22:05 2020
%DIF ADD mixed_unitaries.tex         Thu May  7 17:18:06 2020
...
Campbell \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{Campbell2017,1811.08017} }\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Campbell2017,Campbell2019} }\hspace{0pt}%DIFAUXCMD
}\DIFaddend and Hastings \cite{Hastings2017} have developed a technique for suppressing coherent noise by replacing deterministic gate implementations with a \emph{mixed quantum gate} (MQG) consisting of a randomly sampled ensemble of implementations. They focus on errors in gate compilation, such as those that arise from the Solovey-Kitaev algorithm, for which any lingering approximation errors are generally coherent, even if the underlying native gates are perfect. Different approximate gate compilations of the same target unitary will almost certainly result in different unitary errors. So by selecting from these various compilations at random, the resulting quantum channel becomes a mixture of unitaries \cite{DBLP:journals/corr/cs-CC-0012017}, which can have significantly less coherent error than any single compilation on its own. A very simple example of this reduction in coherent error by mixed quantum gates is illustrated in Fig.~\ref{fig:simple_example}. 

Randomized protocols have a long history of outperforming their deterministic counterparts\cite{Viola2005, Santos2006}. In circuit models, Pauli Frame Randomization \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{Wallman2016, 1803.01818} }\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Wallman2016, 1803.01818, dahlen2018experimental} }\hspace{0pt}%DIFAUXCMD
}\DIFaddend has been shown to reduce coherent errors by introducing randomness in a \DIFdelbegin \DIFdel{pre-compilation step. While Pauli Frame Randomization shares the some favorable properties with MQGs , we will see that MQGs additionally admit a hardware-efficient implementation and numerous useful modifications when they are }\DIFdelend \DIFaddbegin \DIFadd{compilation step. MQGs are an alternative approach to solving this problem. Because they are implemented by randomly drawing from different gate realizations, they only require randomness at run-time, allowing them to be easily implemented in hardware. Moreover, when the construction of MQGs is }\DIFaddend posed as a numerical optimization problem\DIFaddbegin \DIFadd{, numerous useful modifications can be made to augment their performance}\DIFaddend .
...
\DIFaddbegin \label{sec:convex_programs}
\DIFaddend \subsubsection{Generator-exact MQGs} % (fold)
...
\label{sec:sparsity}
\noindent
As a practical consideration, we would also like to regularize our objective function to promote sparse weightings\DIFaddbegin \DIFadd{, resulting in MQGs composed of only a small number of individual gate implementations}\DIFaddend . Control electronics often have a limited amount of waveform memory, and thus it is important to be able to construct MQGs with as few constituent gates as possible. In many machine learning contexts, \DIFaddbegin \DIFadd{a technique called }\DIFaddend lasso regularization \cite{tibshirani1996regression} can be used to enforce \DIFaddbegin \DIFadd{this }\DIFaddend sparsity in solutions. 
\DIFaddbegin 

\DIFaddend Rather than penalizing the $\ell_0$ norm of the solution, which is not convex, lasso regularization penalizes the $\ell_1$ \DIFaddbegin \DIFadd{norm}\DIFaddend , which is \DIFdelbegin \DIFdel{the }\DIFdelend \DIFaddbegin \DIFadd{a }\DIFaddend convex relaxation of the $\ell_0$ norm. However this is insufficient for our purposes, as we already require $\weight$ to be a valid probability distribution, which constrains the $\ell_1$ norm to be equal to $1$.
\DIFdelbegin \DIFdel{Conveniently, the }\DIFdelend \DIFaddbegin 

\DIFadd{The }\DIFaddend problem of enforcing sparsity in such situations has been considered in \cite{NIPS2012_4504} and can be expressed via another convex program that extends Eq.~\eqref{eq:minimization}:
\begin{equation}\DIFdelbegin %DIFDELCMD < \label{eq:minimization_regularization}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \label{eq:minimization_sparsity}
...
\end{split}
\end{equation}
\DIFdelbegin \DIFdel{Here, $\lambda > 0$ }\DIFdelend \DIFaddbegin 

\DIFadd{Here, $\lambda \geq 0$ }\DIFaddend is a tunable parameter that can be used to control the degree of sparsity in the solution. \DIFdelbegin \DIFdel{Increasing }\DIFdelend \DIFaddbegin \DIFadd{If }\DIFaddend $\lambda$ \DIFdelbegin \DIFdel{will }\DIFdelend \DIFaddbegin \DIFadd{is set to zero, the inner program reduces to Eq.~}\eqref{eq:minimization} \DIFadd{and the outer minimization is redundant. If $\lambda$ is very large, each inner minimization problem is minimized for an MQG with a single control, and the outer minimization simply selects the control which has the error generator of smallest norm. While this gives a maximally sparse solution, it doesn't allow for any cancellation of coherent errors.
}

\DIFadd{In general, increasing $\lambda$ decreases the number of controls with large weights, and by removing controls with weights below a threshold probability we can }\DIFaddend increase the sparsity of the solution\DIFdelbegin \DIFdel{, but will generally }\DIFdelend \DIFaddbegin \DIFadd{. Truncating in this way will introduce error in the generator-exactness of the solution. For larger $\lambda$ the required threshold probability decreases, and so the error introduced from truncation can be made smaller. However, larger $\lambda$ will also }\DIFaddend decrease the likelihood \DIFdelbegin \DIFdel{that the solution will be }\DIFdelend \DIFaddbegin \DIFadd{of the optimizer returning a }\DIFaddend generator-exact \DIFaddbegin \DIFadd{solution, and so in this way, tuning $\lambda$ allows us to trade between different sources of error when constructing sparse MQGs}\DIFaddend .
...
\DIFadd{To construct an MQG, there needs to be several different realizations of the desired gate available. To see improved performance, these realizations need to be imperfect in different ways, from drift, miscalibration, or other sources of error. Because the qubit used in our demonstration was coherence-limited, and therefore did not suffer from miscalibration errors, we intentionally introduced coherent errors to showcase our technique.  }\DIFaddend Controls were derived from a single, 10-sample, 50ns Gaussian $X_{\pi/2}$ pulse calibrated via Rabi oscillation. Four intentionally miscalibrated Gaussian pulses, labeled $\mathsf{Pulse1}$ through $\mathsf{Pulse4}$, were derived by scaling the amplitude of the calibrated $X_{\pi/2}$ pulse by a factor $S \in \{1.064, 1.039,0.937, 0.912\}$, respectively. We model their effect on the qubit state as perfectly unitary rotations, $X_{A \pi/2} = \exp{(-i A \pi/2 \sigma_x)}$.
\DIFaddbegin 

 \DIFaddend These gates are then used as in Sec.~\ref{sub:off_diagonals} to construct a Pauli-exact MQG, which samples from the miscalibrated waveforms with probabilities $P\in \{0.307, 0.283, 0.211, 0.199\}$ respectively. Using these probabilities and pulse amplitudes we have plotted the corresponding \DIFdelbegin \DIFdel{Pauli transfer matrices }\DIFdelend \DIFaddbegin \DIFadd{error generators }\DIFaddend in Fig.\DIFdelbegin \DIFdel{\ref{ptms}}\DIFdelend \DIFaddbegin \DIFadd{~\ref{fig:ptms}}\DIFaddend . $\mathsf{Pulse1}$ through $\mathsf{Pulse4}$ show clear coherent errors as off-diagonal elements while the MQG exhibits a clear \DIFdelbegin \DIFdel{supression }\DIFdelend \DIFaddbegin \DIFadd{suppression }\DIFaddend of these off-diagonal elements.
\begin{figure}
\includegraphics[scale=.5]{experimental-ptm.png}
\DIFdelbeginFL %DIFDELCMD < \label{ptms}
%DIFDELCMD < %%%
\DIFdelendFL \caption{\DIFdelbeginFL \DIFdelFL{Pauli transfer matrices }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Hinton diagrams representing the error generators }\DIFaddendFL for a) $\mathsf{Pulse1}$, b) $\mathsf{Pulse2}$, c) $\mathsf{Pulse3}$, d) $\mathsf{Pulse4}$ and e) a Pauli-exact MQG. \DIFaddbeginFL \DIFaddFL{White boxes correspond to positive values, black  boxes correspond to negative values, and the size of the box corresponds to the magnitude of each entry. }\DIFaddendFL The four constituent pulses can be seen to have coherent \DIFdelbeginFL \DIFdelFL{error }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{errors }\DIFaddendFL from the off-diagonal components of their \DIFdelbeginFL \DIFdelFL{Pauli transfer matrices, while the }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{error generators. The }\DIFaddendFL Pauli-exact MQG has been constructed to have minimal coherent error and therefore has \DIFdelbeginFL \DIFdelFL{predominantly }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{only }\DIFaddendFL diagonal elements.}   
\DIFaddbeginFL \label{fig:ptms}
...
\begin{figure}
\DIFdelbeginFL %DIFDELCMD < \includegraphics[scale=.5]{sparisity-versus-parameter.png}
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\columnwidth]{sparisity-versus-parameter_kcy.png}
\DIFaddendFL \caption{Sparsity of MQGs as a function of the \DIFdelbeginFL \DIFdelFL{sparisty }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{sparsity }\DIFaddendFL parameter $\lambda$, with \DIFdelbeginFL \DIFdelFL{higher weight controls omitted to emphasize the low-weight distribution. Numerical values of $\lambda_i$ are: }\DIFdelendFL $\lambda_0=0$ \DIFdelbeginFL \DIFdelFL{, $\lambda_1=6.25$e-05, $\lambda_2=0.0001$, $\lambda_3=0.00025$ }\DIFdelendFL and \DIFdelbeginFL \DIFdelFL{$\lambda_4=0.0005$}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\lambda_1=0.0005$}\DIFaddendFL . As the parameter is increased from zero, the number of controls with low probability increases \DIFaddbeginFL \DIFaddFL{(seen on the plot as the probability mass moving to the left)}\DIFaddendFL . For \DIFdelbeginFL \DIFdelFL{$\lambda_4$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$\lambda_1$ }\DIFaddendFL virtually all of the 750 controls have vanishingly small probability, allowing for the restriction of the MQG to a small number of controls.}
\label{fig:sparsity}
\end{figure}
To generate the initial controls, we use the GRAPE algorithm\cite{Khaneja2005} with N=25 steps and total evolution time of $\pi$ to generate 750 candidate controls. \DIFdelbegin \DIFdel{However, by imposing the sparisty constraint discussed }\DIFdelend \DIFaddbegin \DIFadd{Using the results from }\DIFaddend in Section \ref{sec:sparsity} \DIFdelbegin \DIFdel{, we found MQGs consisting of just }\DIFdelend \DIFaddbegin \DIFadd{we generated MQGs that sample from only }\DIFaddend 10 \DIFaddbegin \DIFadd{of these }\DIFaddend controls. Fig.~\ref{fig:sparsity} shows \DIFdelbegin \DIFdel{that as }\DIFdelend \DIFaddbegin \DIFadd{solutions to Eq.~}\eqref{eq:minimization_sparsity} \DIFadd{for different choices of  }\DIFaddend $\lambda$\DIFdelbegin \DIFdel{(Eq.}%DIFDELCMD < \eqref{eq:minimization_regularization}%%%
\DIFdel{) increases, the support of the MQGs become more peaked, with most of their probability mass concentrating on a smaller number of controls. 
}\DIFdelend \DIFaddbegin \DIFadd{. As $\lambda$ increases, most of the weights in the optimal solution become smaller, resulting in a sparser MQG. 
}
...
\begin{figure}[h]
  \centering
  \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\columnwidth]{figures/1q_robustness_delta.pdf}
%DIFDELCMD <   %%%
...
\section{Conclusion and Future Work}
We have shown numerically that using MQGs can reduce coherent error by more than an order of magnitude in diamond norm, over a wide range of quasi-static values of noise. In addition, we have demonstrated that these approximate controls can be generated through optimal control (GRAPE), and that the minimization problem is tractable. \DIFaddbegin \DIFadd{By implementing four miscalibrated $X_{\frac{\pi}{2}}$ rotations on a superconducting qubit and drawing from them with optimized weights, we see that coherent over- and under- rotations can be averaged away.
}\DIFaddend 
