...
%DIF ADD mixed_unitaries.tex         Tue Apr 28 13:13:02 2020
...
\label{sec:sparsity}
\noindent
As a practical consideration, we would also like to regularize our objective function to promote sparse weightings\DIFaddbegin \DIFadd{, resulting in MQGs with only a small number of gate implementations}\DIFaddend . Control electronics often have a limited amount of waveform memory, and thus it is important to be able to construct MQGs with as few constituent gates as possible. In many machine learning contexts, \DIFaddbegin \DIFadd{a technique called }\DIFaddend lasso regularization \cite{tibshirani1996regression} can be used to enforce \DIFaddbegin \DIFadd{this }\DIFaddend sparsity in solutions. 
\DIFaddbegin 
...
%DIFDELCMD < %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[scale=.5]{sparisity-versus-parameter_kcy.png}
...
\DIFdel{) }\DIFdelend \DIFaddbegin \DIFadd{. As $\lambda$ }\DIFaddend increases, the \DIFdelbegin \DIFdel{support of the MQGs become more peaked, with most of their probability mass concentrating on a smaller number of controls. 
}\DIFdelend \DIFaddbegin \DIFadd{optimal solution contains less controls. 
}
...
\section{Conclusion and Future Work}
We have shown numerically that using MQGs can reduce coherent error by more than an order of magnitude in diamond norm, over a wide range of quasi-static values of noise. In addition, we have demonstrated that these approximate controls can be generated through optimal control (GRAPE), and that the minimization problem is tractable\DIFaddbegin \DIFadd{. By implementing four miscalibrated $X_{\frac{\pi}{2}}$ rotations on a superconducting qubit and drawing from them with optimized weights, we see that coherent over- and under- rotations can be averaged away}\DIFaddend .
